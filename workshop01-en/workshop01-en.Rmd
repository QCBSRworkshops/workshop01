...
.small[
.center[You have just ran a command preceeded by `?`. What happened?]
]

---
# Still within the `R` **console**: a  tip
...
.center[![:scale 40%](https://bookdown.org/ndphillips/YaRrr/images/environmentss.png) ]
...
An object that has just a signle value or unit like a number or a text string is called a **scalar**.
...
**Can you guess what are we going to discuss next?**
...
We have our information stored as a vector, a matrix, or a dataframe in `R`.
...
.center[What happened there? What is that `r NA`?]
...
We can do that! But first, you need to learn about **logical operators**.
