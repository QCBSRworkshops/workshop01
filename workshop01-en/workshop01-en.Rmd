...
##### 4. Install, use `R` package, functions and get help.
...
If the restriction `unable to write on disk` appears when you attempt to open **R Studio** or to install a package.
...
- Right-click on your **R Studio** icon and click on "Execute as Administrator". This will provide file and directory writing rights to **R Studio**.
...
2. **Environment, History, Connections** group; and,
3. **Files, Plot, Packages, Help, Viewer** panels.

<br>
Once you *Open a Script* or *Create a New Script* (*File > New File > R Script* or `Ctrl/Cmd + Shit + N`), a fourth panel will appear.

<br>

.center[Insert figure of the RStudio interface]
...
---
# Still within the `R` **Console**: a command line tip


Use the $\uparrow$ and $\downarrow$ arrow keys to retrieve commands previously run.
...
If you type `?` and `Constants` (as in `?Constants`) and execute it, you will find them, including the one for $\pi$. 

We can then write and execute the following code:
...
<br>
.center[You might have realized what happens when you run commands preceeded by `?`.

We will get into more details about that in a few minutes!]
