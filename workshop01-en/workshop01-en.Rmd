...
- The value on the **right** (_"assigned value"_) is assigned to the name on the **left** (_object_name_) with the assignment operator `<-`. Basically, it represents an arrow pointing where to store the information. 
...
- Each column must be of the same type, but two columns can differs in type. 
- Several statistical functions (e.g. regression) use dataframes.
...
- A single value is called an *atomic value* (becasue it can't be divided)
...
Where "site_id" is the ID of the site, "soil_pH" is the soil pH, "num_sp" is the number of species and the last column is the treatment applied. 
...
- In this course, we focus mainly on vector and data frames.
...
It also work with multiple position with `c()`:
