...
- As of October 2020, only from the CRAN, there are more than 16000 packages!
...
arrows(0, 0, x1 = 1, length = 0, lwd = 4)

# Center of the circle 
points(0,0, pch = 20)

# Add text to indicate the radius 
text(0, .1, "radius = 5 cm", cex = 2)
```
...
`vector` $\rightarrow$ A combination of elements of the same data type. Basic data structure of R; all other objects consist of one or more vectors.

--

`matrix` $\rightarrow$ 2D object generated by combining multiple vectors of the same type. We can do operations on matrices in R (+, -, *, ...).

--

`array` $\rightarrow$ 3D object generated by combining multiple matrices of the same type. Vector = 1D array. Matrix = 2D array.

--

`data.frame` $\rightarrow$ 2D object generated by combining several vectors of any type. Each column must be of the same type, but a dataframe may contain columns of different types. Several statistical functions (eg regression) use dataframes.

--

`list` $\rightarrow$ A list store collections of objects of any types.
...
.comment[**Hint**: Use the `dput` function to obtain the structure of an object.]
````{r}
dput(odd_n)
````

.comment[The output can be copied and pasted to create a new object by using the `structure` function.]
````{r}
structure(c(1, 3, 5, 7, 9))
````

.comment[These functions are really useful to provide a reproductible example for a question on **stackoverflow** for instance (see one more application in the part about data frames) !]
---
# Vector
...
---
# Data frame
.comment[It's possible to rebuild the initial data frame (with some associated metadata as the class of variables) by copying and pasting the previous output:]

````{r}
structure(list(site_id = structure(1:4, .Label = c("A1.01", "A1.02",
"B1.01", "B1.02"), class = "factor"), soil_pH = c(5.6, 7.3, 4.1,
6), num_sp = c(17, 23, 15, 7), treatment = structure(c(1L, 1L,
2L, 2L), .Label = c("Fert", "No_fert"), class = "factor")),
class = "data.frame", row.names = c(NA, -4L))
````
---
# Matrice, Array and List
...
`my_df[1, ]` $\rightarrow$ Extracts the first row


.comment[Note that an empty index select **all** the values.]

`my_df[, 3]` $\rightarrow$ Extracts the third column


`my_df[2, 4]` $\rightarrow$ Extracts the second element of the fourth column


---
# Indexing data frames: examples
`my_df[c(2, 4), ]` $\rightarrow$ Extracts rows 2 and 4

.comment[The examples given so far are also valid for indexing matrices, this is not the case for the following examples.]


`my_df$site_id` $\rightarrow$ Extracts the variable `site_id` from the data frame

`my_df$site_id[2]` $\rightarrow$ Extracts the second value of the variable `siteID` from the data frame


`my_df[c("site_id", "soil_pH")]` $\rightarrow$ Extracts the `site_id` and `soil_pH` variables from the data frame.
...
```
  ...
  .comment[For more details about the `ggplot2` package, see the workshop #3 - [**Introduction to ggplot2**](https://wiki.qcbs.ca/r_workshop3) !!!]
---
class: inverse, center, middle
...
2. Create a unsorted vector of your favorite numbers, then sort your vector in reverse order.
...
Lot of [cheat sheets](https://www.rstudio.com/resources/cheatsheets/) are available online.
