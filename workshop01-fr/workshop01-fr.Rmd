...
.comment[Ces commandes sont particulièrement utiles pour fournir un exemple reproductible dans le cas d'une question sur **stackoverflow** par exemple (voir application d'ici quelques diapos) !]
---
## Vecteur
...
|ID du site|pH sol|# espece|essai|
|---|---|---|---|
|A1.01|5.6|17|Fert|
|A1.02|7.3|23|Fert|
|B1.01|4.1|15|Non_fert|
|B1.02|6.0|7|Non_fert|
...
Dans cette ligne de code, vous **testez une déclaration logique**. Pour chaque entrée dans le `char_vecteur`, `R` vérifie si l'entrée est égale à `bleu` ou non.

```{r}
char_vecteur[char_vecteur == "bleu"]
```

Dans cette ligne de code, vous demandez à `R` d'extraire toutes les valeurs du vecteur `char_vecteur` qui sont exactement égales à `bleu`.
